...
public class ConnectionValidator extends AbstractFormValidator {
	...
	public void validate(Form<?> form) {
		...
		if (!commonName.getValue().equals("")) {
			...
			for (ConnectionModel connection : connections) {
				if (connection.getCommonName() == null)
					continue;
				if (hostname.getValue().equals(connection.getHostname())) 
					for (ConnectionModel connection1 : connections) 
						if (portNumber == connection1.getPort()) {
