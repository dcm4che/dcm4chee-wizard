...
import org.dcm4chee.wizard.war.configuration.basic.validator.DestinationAETitleValidator;
import org.dcm4chee.wizard.war.configuration.basic.validator.ScheduleValidator;
...
public class CreateOrEditForwardSchedulePage extends SecureWebPage {
    ...
    public CreateOrEditForwardSchedulePage(final ModalWindow window, final ForwardScheduleModel fsModel, 
    		final TreeNode fssNode, final TreeNode fsNode) {
    	super();
        ...
        List<String> uniqueAETitles = new ArrayList<String>(DeviceTreeProvider.get().getUniqueAETitles());
        ...
        form.add(new AjaxFallbackButton("submit", new ResourceModel("saveBtn"), form) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                try {
                    ...
                    if (fsModel == null)
                    	DeviceTreeProvider.get().addForwardSchedule(fssNode, destinationAETitleModel.getObject(), schedule);
                    else 
                    	DeviceTreeProvider.get().editForwardSchedule(fssNode, fsNode, destinationAETitleModel.getObject(), schedule);
