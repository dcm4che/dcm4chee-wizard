...
import org.dcm4chee.wizard.war.configuration.basic.validator.CodeValidator;
 ...
 */
public class CreateOrEditDevicePage extends SecureWebPage {
    ...
    public CreateOrEditDevicePage(final ModalWindow window, final DeviceModel deviceModel) {
        ...
        try {
        	if (deviceModel == null) {
        		typeModel = Model.of(DeviceTreeProvider.ConfigurationType.Basic);
	        	...
	        	typeModel = Model.of(deviceModel.getDevice() instanceof ProxyDevice ? 
	        			DeviceTreeProvider.ConfigurationType.Proxy : DeviceTreeProvider.ConfigurationType.Basic);
        ...
        ArrayList<DeviceTreeProvider.ConfigurationType> configurationTypeList = 
        		new ArrayList<DeviceTreeProvider.ConfigurationType>();
        configurationTypeList.add(DeviceTreeProvider.ConfigurationType.Basic); 
        configurationTypeList.add(DeviceTreeProvider.ConfigurationType.Proxy);
        configurationTypeList.add(DeviceTreeProvider.ConfigurationType.Archive);
        DropDownChoice<DeviceTreeProvider.ConfigurationType> typeDropDown = 
        		new DropDownChoice<DeviceTreeProvider.ConfigurationType>("type", typeModel, configurationTypeList);
        ...
        .add(new TextField<String>("title", deviceNameModel)
        		.add(new DeviceNameValidator(deviceNameModel.getObject()))
...
//                    device.re
                    ...
                    if (deviceModel == null)
                    	DeviceTreeProvider.get().addDevice(device);
                    else
                    	DeviceTreeProvider.get().editDevice(device);                   
                    window.close(target);
                } catch (Exception e) {
