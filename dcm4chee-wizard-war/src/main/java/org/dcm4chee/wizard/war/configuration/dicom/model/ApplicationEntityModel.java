...
public class ApplicationEntityModel implements Serializable, ConfigurationNodeModel {
    ...
    transient ApplicationEntity applicationEntity;
	...
	public ApplicationEntityModel(ApplicationEntity applicationEntity) {
		this.deviceName = applicationEntity.getDevice().getDeviceName();
	...
	public void rebuild(Device device) throws ConfigurationException {
		if (device == null)
			device = DeviceTreeProvider.get().getDicomConfigurationProxy().getDevice(deviceName);
		if (applicationEntity == null) 
			applicationEntity = device.getApplicationEntity(aeTitle);	
		for (TransferCapabilityModel transferCapabilityModel : transferCapabilityModels) 
			transferCapabilityModel.rebuild(applicationEntity);
	}
