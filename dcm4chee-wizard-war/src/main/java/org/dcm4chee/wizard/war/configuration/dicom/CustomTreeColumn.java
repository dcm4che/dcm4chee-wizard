 ...
 */
public class CustomTreeColumn extends TreeColumn<ConfigurationTreeNode> {
	...
	public CustomTreeColumn(IModel<String> displayModel) {
		...
		nodeCssClasses.put(TreeNodeType.FOLDER_CONNECTIONS, CustomTreeColumn.folder_connections_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_APPLICATION_ENTITIES, CustomTreeColumn.folder_application_entities_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_TRANSFER_CAPABILITIES, CustomTreeColumn.folder_transfer_capabilities_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_TRANSFER_CAPABILITY_TYPE, CustomTreeColumn.folder_transfer_capability_type_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_FORWARD_RULES, CustomTreeColumn.folder_forward_rules_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_FORWARD_SCHEDULES, CustomTreeColumn.folder_forward_schedules_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_RETRIES, CustomTreeColumn.folder_retries_cssClass);
		nodeCssClasses.put(TreeNodeType.FOLDER_COERCIONS, CustomTreeColumn.folder_coercions_cssClass);
	}

	@Override
	public void populateItem(Item<ICellPopulator<ConfigurationTreeNode>> cellItem, 
			String componentId, IModel<ConfigurationTreeNode> rowModel) {
		super.populateItem(cellItem, componentId, rowModel);
		
		cellItem.add(new AttributeAppender("class", 
				Model.of(nodeCssClasses.get(rowModel.getObject().getType())), " "));
		cellItem.add(new AttributeAppender("title", 
				Model.of(nodeToolTips.get(rowModel.getObject().getType())), " "));
	}
}
