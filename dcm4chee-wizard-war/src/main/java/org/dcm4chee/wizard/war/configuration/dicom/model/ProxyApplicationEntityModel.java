...
public class ProxyApplicationEntityModel extends ApplicationEntityModel {
	...
	public ProxyApplicationEntityModel(ProxyApplicationEntity applicationEntity) {
    ...
    private void setForwardRules(Collection<ForwardRule> forwardRules) {
		forwardRuleModels = new ArrayList<ForwardRuleModel>();
		for (ForwardRule forwardRule : forwardRules) 
			forwardRuleModels.add(new ForwardRuleModel(forwardRule, super.getAETitle()));
	}
	...
	private void setRetries(List<Retry> retries) {
		...
		for (Retry retry : retries) 
			retryModels.add(new RetryModel(applicationEntity, retry));
	}
	...
	private void setCoercions(AttributeCoercions attributeCoercions) {
		...
		for (AttributeCoercion attributeCoercion : attributeCoercions.getAll())
			this.coercionModels.add(new CoercionModel(applicationEntity, attributeCoercion));
	}
