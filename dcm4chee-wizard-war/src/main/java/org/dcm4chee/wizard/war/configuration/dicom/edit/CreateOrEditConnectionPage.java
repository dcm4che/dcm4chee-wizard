...
import org.dcm4chee.wizard.war.configuration.dicom.validator.ConnectionValidator;
import org.dcm4chee.wizard.war.configuration.dicom.validator.HostnameValidator;
...
public class CreateOrEditConnectionPage extends SecureWebPage {
    ...
    public CreateOrEditConnectionPage(final ModalWindow window, final ConnectionModel connectionModel, 
    		final ConfigurationTreeNode connectionsNode) {
        super();
        ...
        form.add(new AjaxFallbackButton("submit", new ResourceModel("saveBtn"), form) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                try {
                    ...
                    if (connectionModel == null)
                    	DeviceTreeProvider.get().addConnection(connectionsNode, connection);
                    else
                    	DeviceTreeProvider.get().editConnection(connectionsNode, connection);
                    window.close(target);
                } catch (Exception e) {
